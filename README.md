...
| `--T`, `--t <datetime start>` | Filter content by start date/time (KST only, use full year like `2025`) |
| `--TT`, `--tt <datetime end>` | [Optional] Filter content by end date/time (KST only, use full year like `2025`) |
...
| `--artisid` | Specify ArtistID (e.g., `--artisid 71,72,73`) — only works in Artis archive |
...
if __name__ == "__main__":
    ...
    async def main() -> Never:
        ...
        """2. after al url parser finsh, collect all data into a class selected_media_list"""
