...
logger = setup_logging("playready_remote_cdm", "light_blue")
...
class RemoteCdm(Cdm):
    ...
    def __init__(self, security_level: int, host: str, secret: str, device_name: str):
        ...
        response = httpx.head(self.host)

        if response.status_code != 200:
            logger.warning(f"Could not test Remote API version [{response.status_code}]")

        server = response.headers.get("Server")
        if not server or "playready serve" not in server.lower():
            logger.warning(f"This Remote CDM API does not seem to be a playready serve API ({server}).")
    ...
    def open(self) -> bytes:
        response = self.__session.get(url=f"{self.host}/{self.device_name}/open")
    ...
    def close(self, session_id: bytes) -> None:
        response = self.__session.get(url=f"{self.host}/{self.device_name}/close/{session_id.hex()}")
        response_json = response.json()

        if response.status_code != 200:
            raise ValueError(f"Cannot Close CDM Session, {response_json['message']} [{response.status_code}]")

    def get_license_challenge(self, session_id: bytes, wrm_header: WRMHeader | str) -> str:
        if not wrm_header:
        ...
        response = self.__session.post(
            ...
            json={"session_id": session_id.hex(), "init_data": wrm_header},
        )
        response_json = response.json()
    ...
    def parse_license(self, session_id: bytes, license_message: str) -> None:
        ...
        response = self.__session.post(
            ...
            json={"session_id": session_id.hex(), "license_message": license_message},
        )
        response_json = response.json()
