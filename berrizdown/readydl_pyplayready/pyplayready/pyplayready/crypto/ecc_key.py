...
class ECCKey:
    ...
    @classmethod
    def construct(cls, private_key: bytes | int):
    ...
    @classmethod
    def loads(cls, data: str | bytes) -> ECCKey:
    ...
    @classmethod
    def load(cls, path: Path | str) -> ECCKey:
    ...
    def dump(self, path: Path | str, private_only=False) -> None:
        if not isinstance(path, (Path, str)):
            raise ValueError(f"Expecting Path object or path string, got {path!r}")
        path = Path(path)
