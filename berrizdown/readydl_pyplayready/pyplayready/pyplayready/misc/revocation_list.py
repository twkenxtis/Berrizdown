...
from pyplayready.system.bcert import BCertCertType, BCertKeyUsage, CertificateChain
...
class RevocationList(_RevocationStructs):
    ...
    def _verify_crl_signatures(crl: Container, data_struct) -> None:
        if isinstance(crl.certificate_chain, bytes) and len(crl.certificate_chain) == 64:
            ...
            signing_cert.verify_chain(check_expiry=True, cert_type=BCertCertType.CRL_SIGNER)
        ...
        if not Crypto.ecc256_verify(public_key=signing_key, data=sign_payload, signature=crl.signature):
    ...
    def _verify_wmdrmnet_wrap_signature(xml: str) -> bool:
        ...
        msdrm_ecc1_params = {
            ...
            "generator": (
                0x8723947FD6A3A1E53510C07DBA38DAF0109FA120,  # gen x
    ...
    def _verify_and_parse(revocation):
        ...
        if list_id is None or not list_id.text:
            raise InvalidRevocationList("<ListID> is either missing or empty")
        ...
        if list_data is None or not list_data.text:
            raise InvalidRevocationList("<ListData> is either missing or empty")

        return RevocationList._parse_list(list_id_uuid, base64.b64decode(list_data.text))

    @classmethod
    def loads(cls, data: str | bytes | ET.Element) -> RevocationList:
    ...
    @classmethod
    def load(cls, path: Path | str) -> RevocationList:
    ...
    def get_by_id(self, uuid: UUID) -> Container | None:
