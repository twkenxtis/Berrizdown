...
from pyplayready import PSSH, __version__
...
async def get_license_challenge(request: web.Request) -> web.Response:
    ...
    return web.json_response({"message": "Success", "data": {"challenge": license_request}}, status=200)
...
async def get_keys(request: web.Request) -> web.Response:
    ...
    return web.json_response({"message": "Success", "data": {"keys": keys_json}})
...
async def authentication(request: web.Request, handler: Handler) -> web.Response:
    ...
    response.headers.update({"Server": f"https://git.gay/ready-dl/pyplayready serve v{__version__}"})
