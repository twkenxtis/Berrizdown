...
from berrizdown.pywidevine.license_protocol_pb2 import FileHashes, LicenseType
...
@click.option(
    ...
    "--type",
    "license_type",
    type=click.Choice(LicenseType.keys(), case_sensitive=False),
    default="STREAMING",
    help="License Type to Request.",
)
@click.option("-p", "--privacy", is_flag=True, default=False, help="Use Privacy Mode, off by default.")
def license_(device_path: Path, pssh: PSSH, server: str, license_type: str, privacy: bool) -> None:
    ...
    if privacy:
        ...
        service_cert_res = requests.post(url=server, data=cdm.service_certificate_challenge)
    ...
    license_res = requests.post(url=server, data=challenge)
...
def create_device(
    ...
    log.info(
        ...
        len(device.client_id.SerializeToString()),
    )
...
@click.pass_context
def export_device(ctx: click.Context, wvd_path: Path, out_dir: Path | None = None) -> None:
    """
