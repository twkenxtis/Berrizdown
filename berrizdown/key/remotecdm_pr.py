...
from pyplayready.remote import remotecdm
...
class Remotecdm_Playready:
    ...
    def make_request_data(self, rcdm: remotecdm.RemoteCdm, session_id: bytes, pssh_input: str) -> str:
        pssh: PSSH = self.get_pssh(pssh_input)
        request_data: str = rcdm.get_license_challenge(session_id, pssh.wrm_headers[0])
        return request_data

    async def get_license_key(self, pssh_input: str, acquirelicenseassertion: str):
        rcdm: remotecdm = self.get_rcdm()
        session_id: bytes = rcdm.open()
        headers = {
            "user-agent": USERAGENT,
            "content-type": "application/octet-stream",
            "acquirelicenseassertion": acquirelicenseassertion,
        }
        request_data: str = self.make_request_data(rcdm, session_id, pssh_input)
        ...
        key_list: list[str] = []
        for key in rcdm.get_keys(session_id):
